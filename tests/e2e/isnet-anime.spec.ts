  ...
  for (const fixture of FIXTURES) {
    test(`should process ${fixture} correctly`, async ({ page }) => {
      ...
      await page.waitForFunction(() => {
        return (
          window.rembg !== undefined &&
          typeof window.rembg.newSession === 'function' &&
          typeof window.rembg.remove === 'function'
        );
      });
      await page.waitForFunction(() => {
        return (
          window.rembg !== undefined &&
          window.rembg.getAvailableModels &&
          window.rembg.getAvailableModels().length > 0
        );
      });
